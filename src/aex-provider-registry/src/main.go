...
func main() {
	...
	tracerShutdown, err := telemetry.InitTracer(context.Background(), "aex-provider-registry", otlpEndpoint)
	if err != nil {
	...
	metricsHandler, err := telemetry.InitMeter("aex-provider-registry")
	if err != nil {
	...
	if cfg.AllowHTTP {
		slog.Warn("HTTP URLs allowed (development mode)")
	}
	...
	handler := telemetry.HTTPMiddleware("aex-provider-registry")(mux)
