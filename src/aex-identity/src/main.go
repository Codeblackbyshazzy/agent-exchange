...
import (
	...
	"github.com/parlakisik/agent-exchange/aex-identity/internal/config"
...
func main() {
	...
	if cfg.MongoURI != "" {
		...
		mongoClient = c
		ms := store.NewMongoStore(c, cfg.MongoDatabase, cfg.MongoCollectionTenants, cfg.MongoCollectionAPIKeys)
