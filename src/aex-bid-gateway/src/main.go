...
func main() {
	...
	tracerShutdown, err := telemetry.InitTracer(context.Background(), "aex-bid-gateway", otlpEndpoint)
	if err != nil {
	...
	metricsHandler, err := telemetry.InitMeter("aex-bid-gateway")
	if err != nil {
	...
	handler := telemetry.HTTPMiddleware("aex-bid-gateway")(mux)
